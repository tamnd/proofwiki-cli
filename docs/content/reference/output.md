...
describes what users get. Pick a format with `-o`, or let pw choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
pw <command> -o table   # aligned columns for reading
pw <command> -o jsonl   # one JSON object per line, for piping
pw <command> -o json    # a single JSON array
pw <command> -o csv     # spreadsheet friendly
pw <command> -o tsv     # tab-separated
pw <command> -o url     # just the URL column
pw <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
pw <command> --fields id,title,url
```
...
```bash
pw <command>            # a table, because this is a terminal
pw <command> | wc -l    # JSONL, because this is a pipe
```

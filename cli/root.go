// Package cli builds the pw command tree on top of the proofwiki library.
package cli
...
type App struct {
	client *proofwiki.Client
	cfg    proofwiki.Config
...
func Root() *cobra.Command {
	app := &App{cfg: proofwiki.DefaultConfig()}

	root := &cobra.Command{
		Use:   "pw",
		Short: "Browse ProofWiki mathematical proofs",
		Long: `pw reads ProofWiki through the MediaWiki API.
It is open and requires no key. Records are returned as table, JSON, JSONL,
CSV, TSV, URL, or raw.

pw is an independent tool and is not affiliated with ProofWiki.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = proofwiki.NewClient(a.cfg)
	return nil
}
